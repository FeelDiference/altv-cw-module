...
namespace MeshHub.Core.GameFiles
{
    ...
    public class RpfFile
    {
        ...
        public static RpfFile CreateNewAtPath(string fullPhysicalPath, RpfEncryption encryption = RpfEncryption.OPEN)
        {
            // Cross-platform creation at an absolute physical path
            if (File.Exists(fullPhysicalPath))
            {
                throw new Exception("File " + fullPhysicalPath + " already exists!");
            }

            System.IO.Directory.CreateDirectory(System.IO.Path.GetDirectoryName(fullPhysicalPath));
            File.Create(fullPhysicalPath).Dispose();

            string rel = System.IO.Path.GetFileName(fullPhysicalPath);
            RpfFile file = new RpfFile(fullPhysicalPath, rel);

            using (var fstream = File.Open(fullPhysicalPath, FileMode.Open, FileAccess.ReadWrite))
            {
                using (var bw = new BinaryWriter(fstream))
                {
                    file.WriteNewArchive(bw, encryption);
